...
public class PostController {
    ...
    @GetMapping("/spider2")
    public String spider2 (Integer type,Integer firstPage,Integer lastPage) throws IOException {
        Integer count = 0;
        String baseurl = "https://www.ygdy8.net";
        for (int i = firstPage; i<=lastPage; i++) {
            String number = String.valueOf(i);
            String url = "https://www.ygdy8.net/html/gndy/oumei/list_"+type.toString()+"_"+number+".html";
            count += spiderOnePage2(baseurl, url);
        }
        String message = "成功爬取"+count+"条信息";
        return message;
    }
    ...
    @GetMapping("/spider/one2")
    public Integer spiderOnePage2(String baseurl, String url) throws IOException {
        ...
        if (content != null) {
            ...
            for (int i = 1; i<=25; i++) {
                ...
                String movieName = elements.text();
                output = output + str + "\n";
                boolean success = spiderDetail2(baseurl+link,"src/detail.html",movieName);
                if (success) {
                    count++;
                }
            }
